...
OUTPUT_PATH = "/mnt/user-data/outputs/stack_animation.mp4"
...
def draw_frame(stack, op, word, member, top_result=None, highlight_idx=None):
    ...
    draw_rounded_rect(draw,
        (STACK_LEFT - 8, STACK_TOP - 8, STACK_LEFT + STACK_W + 8, stack_bottom + 40),
        radius=12, fill=STACK_BG, outline=STACK_BORDER, width=3)

    # ── "Stack" 레이블 ──
    draw.text((STACK_LEFT + STACK_W // 2 - 25, stack_bottom + 10),
    ...
    draw.text((STACK_LEFT, stack_bottom + 48), size_text, font=f_label, fill=INFO_COLOR)
    ...
    # ── 하단 정보 ──
    members = ["이유민", "김효린", "이승주", "김태연", "이예지", "유지민", "이시현", "서지윤"]
    all_words_flat = [
        "벚꽃","버터떡","두쫀쿠",
        "중간고사","딸기라떼","카공",
        "말차","꽃구경","아아",
    ...
    row_text = "팀원 단어: " + " · ".join(set(all_words_flat))
    draw.text((40, HEIGHT - 35), row_text[:80], font=f_label, fill=INFO_COLOR)
...
def declare_animation_frames():
    ...
    full_box = (STACK_LEFT - 8, STACK_TOP - 8,
                STACK_LEFT + STACK_W + 8, stack_bottom + 40)
    code_str = "stack = Stack()  # 선언"
    ...
    def draw_full_grid(d):
        ...
        d.text((STACK_LEFT + STACK_W // 2 - 25, stack_bottom + 10),
               "Stack", font=f_label, fill=STACK_BORDER)
    ...
    while char_idx <= len(code_str):
        img, d = base_img()
        draw_full_grid(d)
        d.text((STACK_LEFT, stack_bottom + 48),
